...
public record MarkStudentsAbsentAtLessonCommand(
    Guid LessonId,
    List<Guid> StudentIds) : ICommand<Unit>;

public class MarkStudentsAbsentAtLessonCommandHandler(
    IScheduleRepository<LessonScheduleEntity> _scheduleRepository,
    IScheduleUnitOfWork _unitOfWork)
    : ICommandHandler<MarkStudentsAbsentAtLessonCommand, Unit>
{
    public async Task<Result<Unit>> Handle(MarkStudentsAbsentAtLessonCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            existSchedule.MarkStudentsAbsent(request.StudentIds);
        ...
        else
        {
            ...
            entity.MarkStudentsAbsent(request.StudentIds);
